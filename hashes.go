...
func CentralHash(icon images4.IconT, hyperPoints []image.Point,
	...
	cube := hyper.CentralCube(vector,
		hyper.Params{
			Min:        0,
			Max:        255, // TODO: recheck. Is that value still relevant for uint16 premultiplied.
...
func CustomPoints(n int) map[image.Point]bool {
	...
	margin := 2
	iSM := images4.IconSize - margin
	if n > images4.IconSize {
		margin = 1
	}
	pts := make(map[image.Point]bool)
	// First point to be in the upper left corner.
	pts[image.Point{margin, margin}] = true
	...
	for len(pts) < n {
		...
		d := make(map[image.Point]float64)
		for x := margin; x < iSM; x++ {
			for y := margin; y < iSM; y++ {
				if _, ok := pts[image.Point{x, y}]; ok {
					continue
				}
				for p := range pts {
					sumDist += 1 / distance(p, image.Point{x, y})
				}
				d[image.Point{x, y}] = 1 / sumDist
				sumDist = 0
			}
		}
	...
	for i := 0; i < 50; i++ {
		for p0 := range pts {
			...
			for x := margin; x < iSM; x++ {
				for y := margin; y < iSM; y++ {
					if _, ok := pts0[image.Point{x, y}]; ok {
						continue
					}
					n := nearest(pts0, image.Point{x, y})
					d[image.Point{x, y}] = distance(image.Point{x, y}, n)
				}
			}
			newPoint := maxKey(d)
			delete(pts, p0)
...
func minKey(m map[image.Point]float64) (key image.Point) {
	minVal := math.Inf(1)
	for k, v := range m {
		if v < minVal {
			key = k
...
func maxKey(m map[image.Point]float64) (key image.Point) {
	maxVal := math.Inf(-1)
	for k, v := range m {
		if v > maxVal {
			key = k
...
func exclude(p image.Point, s map[image.Point]bool) map[image.Point]bool {
	e := make(map[image.Point]bool)
	for k := range s {
		// Skip point p.
		if k == p {
			continue
...
// another point o.
func nearest(s map[image.Point]bool, o image.Point) image.Point {
